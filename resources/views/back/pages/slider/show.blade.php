    ...
    <form action="/back/slider/{{ $show->id }}/update" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <select name="place" id="place">
                @for ($i = 1; $i <= 15; $i++)
                    @if ($show->place == $i)
                        <option name="place" value="{{ $i }}" selected>{{ $i }}</option>
                    @else
        ...
        <div class="mb-6">
            ...
            <div class="w-1/2 mt-4"><img src="{{ asset('storage/slider/thumbnail/' . $show->src) }}" class="rounded-lg">
            </div>
